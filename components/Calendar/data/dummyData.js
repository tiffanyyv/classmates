export const mentorData = [
    {
      title: 'KEvin CLASS',
      startDate: new Date(2022, 2, 24, 6, 35),
      endDate: new Date(2022, 2, 24, 22, 30),
      zoomlink: 'http://garbase.com'
    },
    {
      title: 'Big Kevin CLASS',
      startDate: new Date(2022, 2, 23, 6, 35),
      endDate: new Date(2022, 2, 23, 9, 30),
      zoomlink: 'http://garbase.com'
    },

    {
      title: 'Coolguy Class',
      startDate: new Date(2022, 2, 20, 6, 15),
      endDate: new Date(2022, 2, 20, 9, 30),
      zoomlink: 'https://us02web.zoom.us/j/5773664084?pwd=MlVFWnJodytZdkpTS1UxbEsyc3Nvdz09'
    },
    {
      title: 'React Workshop 1',
      startDate: new Date(2022, 2, 21, 6, 0),
      endDate: new Date(2022, 2, 21, 9, 50),
      zoomlink: 'https://us02web.zoom.us/j/5773664084?pwd=MlVFWnJodytZdkpTS1UxbEsyc3Nvdz09'
    },
    {
      title: 'Fullstack Review 1',
      subject: 'Science',
      teacherName: 'Josh Hertz',
      startDate: new Date(2022, 2, 22, 6, 0),
      endDate: new Date(2022, 2, 22, 9, 30),
      courseThumbnail: 'https://media.geeksforgeeks.org/wp-content/cdn-uploads/20190626123927/untitlsssssed.png',
      zoomLink: 'https://us02web.zoom.us/j/5773664084?pwd=MlVFWnJodytZdkpTS1UxbEsyc3Nvdz09',
      studentList: ['Alex', 'Chris', 'Estevan', 'Kevin', 'Matt', 'Samantha', 'Teresa', 'Tiffany', 'Tristen']
    },
    {
      title: 'React Workshop 2',
      subject: 'Literature',
      teacherName: 'Quinton Maki',
      startDate: new Date(2022, 2, 25, 6, 0),
      endDate: new Date(2022, 2, 25, 8, 30),
      courseThumbnail: 'https://res.cloudinary.com/practicaldev/image/fetch/s--ItM6k2Vz--/c_imagga_scale,f_auto,fl_progressive,h_900,q_auto,w_1600/https://dev-to-uploads.s3.amazonaws.com/uploads/articles/7vx1ntk1reuhqo3tafbo.png',
      zoomLink: 'https://us02web.zoom.us/j/5773664084?pwd=MlVFWnJodytZdkpTS1UxbEsyc3Nvdz09',
      studentList: ['Alex', 'Chris', 'Estevan', 'Kevin', 'Matt', 'Samantha', 'Teresa', 'Tiffany', 'Tristen']
    },
    {
      title: 'How to Write a Meme',
      subject: 'Literature',
      teacherName: 'Ben Bernardy',
      startDate: new Date(2022, 2, 26, 8, 0),
      endDate: new Date(2022, 2, 26, 10, 30),
      courseThumbnail: 'https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcRkuciZ5KT6W7Xv6-WHJxpbpp2dpssC-l5GgpsyUfsiYzH9HdXcR_0I9XG0x9GduSgAKys&usqp=CAU',
      zoomLink: 'https://us02web.zoom.us/j/5773664084?pwd=MlVFWnJodytZdkpTS1UxbEsyc3Nvdz09',
      studentList: ['Alex', 'Chris', 'Estevan', 'Kevin', 'Matt', 'Samantha', 'Teresa', 'Tiffany', 'Tristen']
    },
    {
      title: 'Blue Ocean Kick-off',
      subject: 'History',
      teacherName: 'Quinton Bernardy',
      startDate: new Date(2022, 2, 24, 16, 0),
      endDate: new Date(2022, 2, 24, 17, 30),
      courseThumbnail: 'https://media.bizj.us/view/img/10304667/howtoblueocean*1200xx3864-2183-0-44.jpg',
      zoomLink: 'https://us02web.zoom.us/j/5773664084?pwd=MlVFWnJodytZdkpTS1UxbEsyc3Nvdz09',
      studentList: ['Alex', 'Chris', 'Estevan', 'Kevin', 'Matt', 'Samantha', 'Teresa', 'Tiffany', 'Tristen']
    },
    {
      title: 'Mech Keys 101',
      subject: 'Science',
      teacherName: 'Kevin Kim',
      startDate: new Date(2022, 2, 28, 6, 0),
      endDate: new Date(2022, 2, 28, 7, 30),
      courseThumbnail: 'https://hips.hearstapps.com/amv-prod-gp.s3.amazonaws.com/gearpatrol/wp-content/uploads/2019/11/Mechanical-Keyboard-Buying-Guide-Gear-Patrol-lead-full.jpg',
      zoomLink: 'https://us02web.zoom.us/j/5773664084?pwd=MlVFWnJodytZdkpTS1UxbEsyc3Nvdz09',
      studentList: ['Alex', 'Chris', 'Estevan', 'Matt', 'Samantha', 'Teresa', 'Tiffany', 'Tristen']
    },
    {
      title: 'Positive Mentality',
      subject: 'Language',
      teacherName: 'Matt Chang',
      startDate: new Date(2022, 2, 27, 10, 0),
      endDate: new Date(2022, 2, 27, 15, 30),
      courseThumbnail: 'https://www.healthshots.com/wp-content/uploads/2020/07/think-positive.jpg',
      zoomLink: 'https://us02web.zoom.us/j/5773664084?pwd=MlVFWnJodytZdkpTS1UxbEsyc3Nvdz09',
      studentList: ['Alex', 'Chris', 'Estevan', 'Kevin', 'Samantha', 'Teresa', 'Tiffany', 'Tristen']
    },
    {
      title: 'Positive Mentality',
      subject: 'History',
      teacherName: 'Matt Chang',
      startDate: new Date(2022, 2, 29, 10, 0),
      endDate: new Date(2022, 2, 29, 14, 30),
      courseThumbnail: 'https://www.healthshots.com/wp-content/uploads/2020/07/think-positive.jpg',
      zoomLink: 'https://us02web.zoom.us/j/5773664084?pwd=MlVFWnJodytZdkpTS1UxbEsyc3Nvdz09',
      studentList: ['Alex', 'Chris', 'Estevan', 'Kevin', 'Samantha', 'Teresa', 'Tiffany', 'Tristen']
    },
]



export const menteeData = [
  {
    title: 'KEvin CLASS',
    startDate: new Date(2022, 2, 24, 9, 35),
    endDate: new Date(2022, 2, 24, 10, 30),
    zoomlink: 'http://garbase.com'
  },
  {
    title: 'Big Kevin CLASS',
    startDate: new Date(2022, 2, 23, 15, 35),
    endDate: new Date(2022, 2, 23, 17, 30),
    zoomlink: 'http://garbase.com'
  },

  {
    title: 'Coolguy Class',
    startDate: new Date(2022, 2, 20, 6, 15),
    endDate: new Date(2022, 2, 20, 9, 30),
    zoomlink: 'https://us02web.zoom.us/j/5773664084?pwd=MlVFWnJodytZdkpTS1UxbEsyc3Nvdz09'
  },
  {
    title: 'React Workshop 1',
    startDate: new Date(2022, 2, 21, 6, 0),
    endDate: new Date(2022, 2, 21, 9, 50),
    zoomlink: 'https://us02web.zoom.us/j/5773664084?pwd=MlVFWnJodytZdkpTS1UxbEsyc3Nvdz09'
  },
  {
    title: 'Fullstack Review 1',
    subject: 'Science',
    teacherName: 'Josh Hertz',
    startDate: new Date(2022, 2, 22, 6, 0),
    endDate: new Date(2022, 2, 22, 9, 30),
    courseThumbnail: 'https://media.geeksforgeeks.org/wp-content/cdn-uploads/20190626123927/untitlsssssed.png',
    zoomLink: 'https://us02web.zoom.us/j/5773664084?pwd=MlVFWnJodytZdkpTS1UxbEsyc3Nvdz09',
    studentList: ['Alex', 'Chris', 'Estevan', 'Kevin', 'Matt', 'Samantha', 'Teresa', 'Tiffany', 'Tristen']
  },
  {
    title: 'React Workshop 2',
    subject: 'Literature',
    teacherName: 'Quinton Maki',
    startDate: new Date(2022, 2, 25, 6, 0),
    endDate: new Date(2022, 2, 25, 8, 30),
    courseThumbnail: 'https://res.cloudinary.com/practicaldev/image/fetch/s--ItM6k2Vz--/c_imagga_scale,f_auto,fl_progressive,h_900,q_auto,w_1600/https://dev-to-uploads.s3.amazonaws.com/uploads/articles/7vx1ntk1reuhqo3tafbo.png',
    zoomLink: 'https://us02web.zoom.us/j/5773664084?pwd=MlVFWnJodytZdkpTS1UxbEsyc3Nvdz09',
    studentList: ['Alex', 'Chris', 'Estevan', 'Kevin', 'Matt', 'Samantha', 'Teresa', 'Tiffany', 'Tristen']
  },
  {
    title: 'How to Write a Meme',
    subject: 'Literature',
    teacherName: 'Ben Bernardy',
    startDate: new Date(2022, 2, 26, 8, 0),
    endDate: new Date(2022, 2, 26, 10, 30),
    courseThumbnail: 'https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcRkuciZ5KT6W7Xv6-WHJxpbpp2dpssC-l5GgpsyUfsiYzH9HdXcR_0I9XG0x9GduSgAKys&usqp=CAU',
    zoomLink: 'https://us02web.zoom.us/j/5773664084?pwd=MlVFWnJodytZdkpTS1UxbEsyc3Nvdz09',
    studentList: ['Alex', 'Chris', 'Estevan', 'Kevin', 'Matt', 'Samantha', 'Teresa', 'Tiffany', 'Tristen']
  },
  {
    title: 'Blue Ocean Kick-off',
    subject: 'History',
    teacherName: 'Quinton Bernardy',
    startDate: new Date(2022, 2, 24, 16, 0),
    endDate: new Date(2022, 2, 24, 17, 30),
    courseThumbnail: 'https://media.bizj.us/view/img/10304667/howtoblueocean*1200xx3864-2183-0-44.jpg',
    zoomLink: 'https://us02web.zoom.us/j/5773664084?pwd=MlVFWnJodytZdkpTS1UxbEsyc3Nvdz09',
    studentList: ['Alex', 'Chris', 'Estevan', 'Kevin', 'Matt', 'Samantha', 'Teresa', 'Tiffany', 'Tristen']
  },
  {
    title: 'Mech Keys 101',
    subject: 'Science',
    teacherName: 'Kevin Kim',
    startDate: new Date(2022, 2, 28, 6, 0),
    endDate: new Date(2022, 2, 28, 7, 30),
    courseThumbnail: 'https://hips.hearstapps.com/amv-prod-gp.s3.amazonaws.com/gearpatrol/wp-content/uploads/2019/11/Mechanical-Keyboard-Buying-Guide-Gear-Patrol-lead-full.jpg',
    zoomLink: 'https://us02web.zoom.us/j/5773664084?pwd=MlVFWnJodytZdkpTS1UxbEsyc3Nvdz09',
    studentList: ['Alex', 'Chris', 'Estevan', 'Matt', 'Samantha', 'Teresa', 'Tiffany', 'Tristen']
  },
  {
    title: 'Positive Mentality',
    subject: 'Language',
    teacherName: 'Matt Chang',
    startDate: new Date(2022, 2, 27, 10, 0),
    endDate: new Date(2022, 2, 27, 15, 30),
    courseThumbnail: 'https://www.healthshots.com/wp-content/uploads/2020/07/think-positive.jpg',
    zoomLink: 'https://us02web.zoom.us/j/5773664084?pwd=MlVFWnJodytZdkpTS1UxbEsyc3Nvdz09',
    studentList: ['Alex', 'Chris', 'Estevan', 'Kevin', 'Samantha', 'Teresa', 'Tiffany', 'Tristen']
  },
  {
    title: 'Positive Mentality',
    subject: 'History',
    teacherName: 'Matt Chang',
    startDate: new Date(2022, 2, 29, 10, 0),
    endDate: new Date(2022, 2, 29, 14, 30),
    courseThumbnail: 'https://www.healthshots.com/wp-content/uploads/2020/07/think-positive.jpg',
    zoomLink: 'https://us02web.zoom.us/j/5773664084?pwd=MlVFWnJodytZdkpTS1UxbEsyc3Nvdz09',
    studentList: ['Alex', 'Chris', 'Estevan', 'Kevin', 'Samantha', 'Teresa', 'Tiffany', 'Tristen']
  },
]
